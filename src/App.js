...
const Portfolio = () => {
  ...
  // ==========================================
  // OPTIMIZED SCROLL HANDLER - DEBOUNCED
  // ==========================================
  useEffect(() => {
    const handleScroll = () => {
  ...
  const socialLinks = [
    { name: 'Instagram', subtext: '@dhyey.237', icon: Instagram, link: 'https://instagram.com/dhyey.237' },
    { name: 'Upwork', subtext: 'Hire Me', icon: ExternalLink, link: '#' },
    { name: 'Stock Images', subtext: 'Portfolio', icon: FileImage, link: '#' },
    { name: 'WhatsApp', subtext: 'Chat Now', icon: MessageCircle, link: 'https://wa.me/919879672277' }
  ];
    ...
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
        ...
        <div className="flex gap-2 md:gap-4 items-center">
          {[
            ...
            <button 
              ...
              className={`relative px-4 md:px-6 py-2.5 rounded-full smooth-transition ${
      ...
      <section id="hero" className="relative min-h-screen flex flex-col items-center justify-center px-4 overflow-hidden pt-32 pb-20">
        ...
        <div className="relative z-10 max-w-5xl w-full mx-auto text-center">
          ...
          <div className="max-w-3xl mx-auto mb-12 px-8 py-10 bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-2xl opacity-0 animate-[fadeIn_0.8s_ease-out_0.6s_forwards] smooth-transition hover:bg-white/8">
          ...
          <div className="flex flex-wrap justify-center gap-4 opacity-0 animate-[fadeIn_0.8s_ease-out_0.8s_forwards]">
            {keywords.map((keyword, idx) => (
              <div 
                key={idx} 
                className="group px-6 py-3 bg-white/5 backdrop-blur-xl rounded-full border border-white/10 smooth-transition hover:bg-white/10 hover:scale-110 hover:border-white/20 cursor-pointer"
              >
                <div className="flex items-center gap-3">
                  <keyword.icon className="w-5 h-5 smooth-transition group-hover:scale-125 group-hover:rotate-12" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {skills.map((skill, index) => {
              ...
              return (
                <div 
                  key={skill.id} 
                  className="skill-card group relative cursor-pointer h-[70vh] min-h-[500px]" 
                ...
                >
                  <div className="relative h-full rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-60 group-hover:opacity-80 smooth-transition" />
                    
                    <div className="skill-blur absolute inset-0" />
                    
                    <div className={`absolute bottom-0 inset-x-0 p-6 smooth-transition ${
                      hoveredSkill === skill.id ? 'translate-y-4 opacity-0' : 'translate-y-0 opacity-100'
                    }`}>
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center p-6 smooth-transition ${
                      hoveredSkill === skill.id ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                    }`}>
                      <div className="bg-black/50 backdrop-blur-xl rounded-3xl p-6 border border-white/20 w-full max-w-sm">
                        ...
                        <div className="space-y-3">
                          {skill.items.map((item, idx) => (
                            <div 
                              key={idx} 
                              className="text-sm bg-white/10 rounded-xl px-4 py-3 border border-white/20 text-center font-medium smooth-transition hover:bg-white/20 hover:scale-105"
                            >
                              {item}
                            </div>
                          ))}
        ...
        <div className="w-full max-w-[1800px] mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-5 md:gap-6 mb-24">
            {tools.map((tool, idx) => (
              <div 
                key={idx} 
                className="group relative px-8 py-6 bg-white/5 backdrop-blur-2xl rounded-2xl border border-white/10 cursor-pointer smooth-transition hover:scale-110 hover:bg-white/10 hover:shadow-2xl hover:border-white/20" 
                onMouseEnter={() => setHoveredTool(tool.name)} 
                onMouseLeave={() => setHoveredTool(null)}
              >
                <div className="relative z-10">
                  <div className="text-5xl mb-3 smooth-transition group-hover:scale-125">{tool.icon}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
            {socialLinks.map((social, idx) => (
              <a 
                ...
                className="group relative p-8 bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/10 smooth-transition hover:bg-white/10 hover:scale-105 hover:border-white/20 text-center overflow-hidden"
              >
                <div className="relative z-10">
                  <social.icon className="w-12 h-12 mx-auto mb-4 smooth-transition group-hover:scale-110" />
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {IMAGES.gallery.map((image, idx) => (
              <div 
                key={idx} 
                className="gallery-item relative rounded-2xl overflow-hidden cursor-pointer border border-white/5 hover:border-white/20"
              >
                <div className="relative w-full" style={{ paddingTop: '133%' }}>
                  <img 
                    src={image}
                    alt={`Gallery ${idx + 1}`} 
                    className="gallery-image absolute inset-0 w-full h-full object-cover"
                    loading="lazy"
                  />
                  <div className="gallery-overlay absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0" />
                  <div className="gallery-label absolute bottom-4 left-4 right-4 transform translate-y-8 opacity-0">
      ...
      <section id="contact" className="relative min-h-screen py-20 md:py-32 px-4 bg-gradient-to-b from-black via-purple-900/10 to-black">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="p-8 bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/10 smooth-transition hover:bg-white/8 hover:border-white/20">
              ...
              <div className="space-y-4">
                <input 
                  ...
                  className="w-full px-6 py-4 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 focus:border-white/30 focus:bg-white/8 focus:outline-none smooth-transition placeholder-white/40" 
                />
                <input 
                  type="email" 
                ...
                <textarea 
                  ...
                  className="w-full px-6 py-4 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 focus:border-white/30 focus:bg-white/8 focus:outline-none smooth-transition placeholder-white/40 resize-none" 
                />
                <button 
                  onClick={handleClientSubmit} 
                  className="w-full px-6 py-4 bg-gradient-to-r from-purple-600 to-pink-600 rounded-2xl font-bold smooth-transition hover:scale-105 hover:shadow-2xl hover:shadow-purple-500/50"
                >
                  Send via WhatsApp
                </button>
              </div>
            ...
            <div className="p-8 bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/10 smooth-transition hover:bg-white/8 hover:border-white/20">
              ...
              <div className="space-y-4">
                ...
                <button 
                  ...
                  className="w-full px-6 py-4 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-2xl font-bold smooth-transition hover:scale-105 hover:shadow-2xl hover:shadow-cyan-500/50"
                >
                  Send Query
                </button>
              </div>
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-8 mb-6">
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer" 
              className="text-white/40 smooth-transition hover:text-white hover:scale-125"
